class Circle {
    ...
    resolveCollisionWithElasticity(circleA, circleB) {
      ...
      if (xVelocityDiff * xDist + yVelocityDiff * yDist >= 0) {
        ...
        let initialRotatedVelocityOfA = this.getRotatedVelocities(circleA.velocity, angle);
        let initialRotatedVelocityOfB = this.getRotatedVelocities(circleB.velocity, angle);
  
        // Velocity after 1d collision newtonian equation
        let calculatedVelocityBeforeRotationA = {
          x: (initialRotatedVelocityOfA.x * (massOfCircleA - massOfCircleB)) / (massOfCircleA + massOfCircleB) + (initialRotatedVelocityOfB.x * 2 * massOfCircleB) / (massOfCircleA + massOfCircleB),
          y: initialRotatedVelocityOfA.y
        };
        let calculatedVelocityBeforeRotationB = {
          x: (initialRotatedVelocityOfB.x * (massOfCircleA - massOfCircleB)) / (massOfCircleA + massOfCircleB) + (initialRotatedVelocityOfA.x * 2 * massOfCircleB) / (massOfCircleA + massOfCircleB),
          y: initialRotatedVelocityOfB.y
        };
  
        // Final velocity after rotating axis back to original location
        let finalVelocityA = this.getRotatedVelocities(calculatedVelocityBeforeRotationA, -angle);
        let finalVelocityB = this.getRotatedVelocities(calculatedVelocityBeforeRotationB, -angle);
