class Carousel {
  ...
  init() {
    this.wrapperElement = this.containerElement.querySelector(
      ".carousel-wrapper"
    );
  ...
  updateIndicatorImages() {
    for (let i = 0; i < this.indicatorButtons.length; i++) {
      if (this.counter === this.indicatorButtons[i].index + 1) {
        this.indicatorButtons[i].className = "disabled";
      } else {
        this.indicatorButtons[i].className = "";
      }
    }
  }
}
